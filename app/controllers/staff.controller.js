...
exports.update = (req, res) => {
  ...
  Staff.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot update Staff with id=${id}. Staff was not found!`,
        });
...
exports.delete = (req, res) => {
  ...
  Staff.findByIdAndRemove(id)
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot delete Staff with id=${id}. Staff was not found!`,
        });
      } else {
        res.send({ message: " Staff was deleted successfully!" });
      }
    })
    .catch((err) => {
      res.status(500).send({ message: "Could not delete Staff with id=" + id });
    });
};
